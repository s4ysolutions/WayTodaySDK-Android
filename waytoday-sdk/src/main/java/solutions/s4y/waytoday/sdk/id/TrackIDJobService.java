...
public class TrackIDJobService extends JobIntentService {
    ...
    public static String getTid(Context context) {
        ...
        return preferences.getString("solutions.s4y.waytoday.sdk.tid", "");
    }

    public static boolean hasTid(Context context) {
        return !"".equals(getTid(context));
    }

    private static void setTid(Context context, String tid) {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        preferences.edit().putString("solutions.s4y.waytoday.sdk.tid", tid).apply();
    }
    ...
    protected void onHandleWork(@NonNull Intent intent) {
        ...
        sFailed = false;
        if (isConnected()) {
            String prevId = intent.getStringExtra(EXTRA_PREVID);
            ...
            try {
                ...
                reportFail(e);
                String oldtid = getTid(this);
                sFailed = !"".equals(oldtid);
            }
        } else {
            reportFail(new Exception("No connection"));
            String oldtid = getTid(this);
            sFailed = !"".equals(oldtid);
        }
        setProgress(false);
